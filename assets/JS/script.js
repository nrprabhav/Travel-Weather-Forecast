...
if (searchHistory == null) {
    ...
    let queryCoordURL = "https://api.openweathermap.org/geo/1.0/direct?q=" + cityName + "&limit=1&appid=" + APIKey;
...
function getCoordinatesAndForecast(queryCoordURL) {
    ...
    $.ajax({
        ...
        let queryForecastURL = "https://api.openweathermap.org/data/2.5/forecast?lat=" + response[0].lat + "&lon=" + response[0].lon + "&appid=" + APIKey;
...
function displayCurrentWeather(response) {
    ...
    $("#weather-icon").attr("src", "https://openweathermap.org/img/wn/" + response.list[0].weather[0].icon + ".png");
...
function displayForecast(response) {
    ...
    for (let i = 1; i < 6; i++) {
        ...
        $("#weather-icon"+i).attr("src", "https://openweathermap.org/img/wn/" + response.list[i].weather[0].icon + ".png");
...
$("#search-button").on("click", function (event) {
    ...
    let queryCoordURL = "https://api.openweathermap.org/geo/1.0/direct?q=" + $("#search-input").val().trim() + "&limit=1&appid=" + APIKey;
